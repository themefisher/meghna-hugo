...
### Load the packages
```{r echo=TRUE}
packages = c('tidyverse', 'tidymodels', 'ggplot2', 'skimr', 'naniar', 'kableExtra', 'cld3','dplyr')
...
listings <- all_listings %>% filter(all_listings$description_lang == "en")
```
...
rcpPreprocess <- recipe(review_scores_rating~., data = listings5) %>%
  ...
  step_rm(host_since) %>%
  step_mutate(neighbourhood_cleansed2 = neighbourhood_cleansed) %>% #duplicate column
  step_other(all_nominal(), -neighbourhood_cleansed, threshold = 0.05,
             other = "Others") #lump factor levels with <5% as "Others", except the original neighbourhood_cleansed column
